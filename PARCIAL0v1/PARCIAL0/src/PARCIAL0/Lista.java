...
public class Lista<E> implements ILista<E> {
    ...
    public void Ordenar() {
        if (primero != null) {
            INodo<E> eleLista = primero;
            INodo<E> aux = primero.getSiguiente();
            eleLista.setSiguiente(null);
            while (aux != null) {
                while (eleLista != null) {
                    if (aux.getEtiqueta().compareTo(eleLista.getSiguiente().getEtiqueta()) < 0) {
                        eleLista.setSiguiente(aux);

                        eleLista = eleLista.getSiguiente();
                        aux = aux.getSiguiente();
                        eleLista.setSiguiente(null);
